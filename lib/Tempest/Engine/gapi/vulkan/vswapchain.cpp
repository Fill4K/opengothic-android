...
#elif defined(__ANDROID__)
  // on Android, presentation support is universal: no per-queue query exists
  (void)device; (void)queueFamilyIndex;
...
void VSwapchain::reset() {
  ...
  // on Android the OS can destroy and re-create the native window (app goes to background),
  // so the vulkan surface has to be re-created as well
  cleanupSurface();
  surface = createSurface(device.instance, hwnd);
#endif
  createSwapchain(device);
...
VkSurfaceKHR VSwapchain::createSurface(VkInstance instance, void* hwnd) {
  ...
  createInfo.window = reinterpret_cast<ANativeWindow*>(AndroidApi::nativeWindow());
  if(createInfo.window==nullptr || vkCreateAndroidSurfaceKHR(instance, &createInfo, nullptr, &ret)!=VK_SUCCESS)
    throw std::system_error(Tempest::GraphicsErrc::NoDevice);
#else
#warning "wsi for vulkan not implemented on this platform"
#endif
...
#ifdef __ANDROID__
  // engine does not implement pre-rotation; let the Android compositor rotate the image
...
#ifdef __ANDROID__
    // render at the launcher-selected height; the compositor upscales the image on present
    VkExtent2D ext = capabilities.currentExtent;
    const uint32_t targetH = uint32_t(Tempest::AndroidApi::renderHeightHint());
    if(targetH>0 && ext.height>targetH) {
      ext.width  = uint32_t((uint64_t(ext.width)*targetH)/ext.height) & ~1u;
      ext.height = targetH;
      ext.width  = std::clamp(ext.width,  capabilities.minImageExtent.width,  capabilities.maxImageExtent.width );
      ext.height = std::clamp(ext.height, capabilities.minImageExtent.height, capabilities.maxImageExtent.height);
      }
    return ext;
#else
