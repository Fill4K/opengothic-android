...
void DialogMenu::mouseDownEvent(MouseEvent &event) {
  ...
  if(event.button==MouseEvent::ButtonLeft) {
    if(isChoiceMenuActive()) {
      // pick the row under the tap: mirror the paintChoice() layout
      const float scale = Gothic::interfaceScale(this);
      const int   padd  = int(20*scale);
      const int   dw    = std::min(w(), int(600*scale));
      auto& fnt = Resources::dialogFont(scale);
      int dh = padd*2;
      for(size_t i=0; i<choice.size(); ++i)
        dh += fnt.textSize(dw-padd, choice[i].title).h;
      const int y0 = h()-dh-int(20*scale);
      const int x0 = (w()-dw)/2;
      int rowTop = y0+padd;
      for(size_t i=0; i<choice.size(); ++i) {
        const int rowH = fnt.textSize(dw-padd, choice[i].title).h;
        if(event.x>=x0 && event.x<x0+dw && event.y>=rowTop && event.y<rowTop+rowH) {
          dlgSel = i;
          update();
          onSelect();
          return;
      ...
      // tap outside of the choice list: do not confirm anything by accident
      return;
      }
    onSelect();
    }
