...
void GameMenu::mouseDownEvent(MouseEvent& event) {
  ...
  for(uint32_t i=0; i<zenkit::IMenu::item_count; ++i) {
    auto& hItem = hItems[i];
    auto& item  = hItem.handle;
    if(item==nullptr || !hItem.visible || !isSelectable(item) || !isEnabled(item))
      continue;
    const int32_t dimx = (item->dim_x!=-1) ? item->dim_x : 8192;
    const int32_t dimy = (item->dim_y!=-1) ? item->dim_y : 750;
    const int x   = int(float(w()*item->pos_x)/scriptDiv);
    const int y   = int(float(h()*item->pos_y)/scriptDiv);
    const int szX = int(float(w()*dimx)/scriptDiv);
    const int szY = int(float(h()*dimy)/scriptDiv);
    const auto at = event.pos();
    if(at.x<x || at.x>=x+szX || at.y<y || at.y>=y+szY)
      continue;
    if(curItem!=i) {
      Gothic::inst().emitGlobalSound(Gothic::inst().loadSoundFx("MENU_BROWSE"));
      setSelection(int(i), 1);
    ...
    if(curItem==i) {
      ...
      exec(hItem, 0, KeyCodec::ActionGeneric);
      }
    return;
    }
  }
