...
MainWindow::MainWindow(Device& device)
  ...
  setupUi();
#if defined(__MOBILE_PLATFORM__)
  mobileUi.setKeyCallback([this](Tempest::Event::KeyType k, bool down) {
    Tempest::KeyEvent ev(k, Tempest::Event::M_NoModifier, down ? Tempest::Event::KeyDown : Tempest::Event::KeyUp);
    if(down)
      keyDownEvent(ev); else
      keyUpEvent(ev);
    });
#ifdef __ANDROID__
  Tempest::AndroidApi::setJoystickCallback(&TouchInput::joystickState);
#endif
#endif
...
#if defined(__MOBILE_PLATFORM__)
  {
  // touch buttons: visible during gameplay and inventory, hidden in menus and dialogs
  const bool uiVisible = Gothic::inst().isInGame() && !dialogs.isActive() && !rootMenu.isActive();
  // virtual joystick: gameplay only, so menu/dialog taps are plain clicks
  const bool joystick  = uiVisible && inventory.isOpen()==InventoryMenu::State::Closed;
  mobileUi.setGameplayMode(uiVisible);
#ifdef __ANDROID__
  Tempest::AndroidApi::setGameplayMode(joystick);
#endif
  }
#endif
  Painter p(event);
...
void MainWindow::tickMouse(uint64_t dt) {
...
#ifdef __ANDROID__
  const bool enableMouse = true; // touch camera control is always on
#else
  ...
  float mouseSensitivity           = Gothic::inst().settingsGetF("GAME","mouseSensitivity")/MouseUtil::mouseSysSpeed();
#ifdef __ANDROID__
  if(mouseSensitivity<=0.f)
    mouseSensitivity = 0.5f/MouseUtil::mouseSysSpeed();
#endif
